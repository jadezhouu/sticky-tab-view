...
import { cancelAnimation, makeMutable, SharedValue, withSpring } from 'react-native-reanimated';
...
export interface ElasticScrollViewHandle {
  scrollTo: (offset: { x: number; y: number }, animated?: boolean) => Promise<void>;
  scroll: (offset: { x: number; y: number }, animated?: boolean) => Promise<void>;
}
 ...
 * @public
 */
const _ElasticScrollView = React.forwardRef<ElasticScrollViewHandle, TElasticScrollViewProps>(
  function ElasticScrollView(userProps, ref) {
    ...
    const xOffsetRef = useRef<SharedValue<number>>(props.contentOffset?.x ?? makeMutable(0));
    const yOffsetRef = useRef<SharedValue<number>>(props.contentOffset?.y ?? makeMutable(0));
    const viewportWidthRef = useRef<SharedValue<number>>(props.size?.width ?? makeMutable(0));
    const viewportHeightRef = useRef<SharedValue<number>>(props.size?.height ?? makeMutable(0));
    const contentWidthRef = useRef<SharedValue<number>>(props.contentSize?.width ?? makeMutable(0));
    const contentHeightRef = useRef<SharedValue<number>>(
      props.contentSize?.height ?? makeMutable(0),
    );
    const focusRef = useRef<SharedValue<TDirection>>(props.focus ?? makeMutable<TDirection>(false));
    const currentPageRef = useRef<SharedValue<number>>(props.currentPage ?? makeMutable(0));
    ...
    const y: SharedValue<number> = context.contentOffset?.y ?? yOffsetRef.current;
